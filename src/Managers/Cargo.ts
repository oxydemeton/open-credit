...
export async function crawlCargoLock(path: string, requestCargoApi = false): Promise<Module[]> {
    ...
    for (let i = 0; i < pack.length; i++) { 
        if(!requestCargoApi){
            ...
            const response = await fetch("https://crates.io/api/v1/crates/"+mod_base.name)
            if(!response.ok) {
            ...
            if(crate.description)   mod.description     = crate.description.toString()
            if(crate.documentation) mod.documentation   = crate.documentation.toString()
